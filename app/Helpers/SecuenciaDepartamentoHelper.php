...
class SecuenciaDepartamentoHelper
{
    ...
    public static function generateSequenceDepartment($idArea, $siglaDepartamento)
    {
        // Se obtiene la última secuencia del departamento por área
        $lastSequence = Departamento::where("id_area", $idArea)
            ->orderBy("id", "desc")
        ...
        $newSequence = sprintf("%03d-%s", $sequenceNumber, $siglaDepartamento);

        return $newSequence;
    }

    // Para actualizar el campo secuencia_area cuando la sigla cambia (ej. DA-001)
    public static function updateSequenceDepartment($departamento, $newSiglaDerpatamento) {
        // Si ha cambiado, se conserva la secuencia y se actualiza solo la sigla
        $currentSequence = $departamento->secuencia_departamento;
        ...
        $updateSequence = sprintf('%03d-%s', $sequenceNumber, $newSiglaDerpatamento);
